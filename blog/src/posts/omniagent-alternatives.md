...
| Munder Difflin | Our local desktop app, described below. | You want many CLIs working as one team on your own machine. |
...
Munder Difflin is ours, so weigh this entry with that in mind. It is a desktop app for macOS, Windows and Linux where every agent is a real CLI in a real terminal, across twelve providers including Claude Code, Codex, Gemini CLI, Copilot and Cursor. Michael, your clone, is the orchestrator you brief. Each agent keeps durable notes in its own `memory.md` that every other agent can read. You can set a token budget for the whole floor and a cap per agent, and a circuit breaker steers an agent that loops or burns tokens too fast.

It is not a like for like swap. It has nothing like Omnigent's layered policies, with rules set server wide, per agent and per session, and your laptop has to stay on because the agents run there. Auto Mode, which skips each CLI's own permission prompts, is on by default in 0.5.2, so turn it off if you want to approve tool calls yourself. The classic office is open source under MIT, and the paid plans are on the [pricing page](https://munderdiffl.in/#pricing).
